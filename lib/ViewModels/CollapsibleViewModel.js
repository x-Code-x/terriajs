 ...
 * @param {Boolean} [isOpen] Whether to start open or closed (default is closed).
*/
var CollapsibleViewModel = function(name, isOpen) {
    this.name = defaultValue(name, '');
    this.isOpen = defaultValue(isOpen, false);
...
CollapsibleViewModel.prototype.renderCollapsible = function() {
    ...
    // var collapsibleContainer = this.container.getElementsByClassName('chart-inject')[0];
    // ALSO NOTE CHILDREN IS NOT IE-SUPPORTED
    var collapsibleContainer = this.container.children[1].children[1].children[0];
    // move back existing content
    var nextNode;
    // TODO: Might as well just loop over this.wrapper.children right?
